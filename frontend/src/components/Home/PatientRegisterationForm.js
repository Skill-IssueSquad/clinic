...
const PatientRegisterationForm = () => {
  ...
  const validateForm = async (formData) => {
    ...
    if(formData.mobileNumber && !validator.isMobilePhone(formData.mobileNumber)){
      errors.mobileNumber = "Mobile number is invalid";
    }
    if(formData.emergencyContactNumber && !validator.isMobilePhone(formData.emergencyContactNumber)){
    ...
    if(Object.keys(errors).length===0){
      try{
        ...
        const response = await fetch('/account/registerPatient', {method: 'POST', 
        headers: {
  ...
  return (
    ...
    <MDBRow >
        ...
        <MDBCardBody className='d-flex flex-column'>
            ...
            <h6 className="fw-normal my-4 pb-1" style={{letterSpacing: '1px'}}>Register now and get access to medical advice.</h6>
        <form onSubmit={handleSubmit}>
            ...
            />
            Female
            <label style={{ marginRight: '.5rem', marginLeft: '60px'}} >Date of Birth</label>
            <input
                type="date"
                name="dateOfBirth"
            ...
            {invalid && <h7 style={{color:'red'}} >{genderErrorMessage}</h7>}
            <br/>
