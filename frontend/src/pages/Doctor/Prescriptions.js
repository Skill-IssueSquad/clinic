...
const PrescriptionsMultiLevelFilterTable = () => {
  let show = false;
  ...
  var columns = location.state.columns;
  var API_GET_URL = location.state.API_GET_URL;
  const initFilter = {};
  columns.forEach((key) => {
    initFilter[key] = "";
  });
  ...
  const handleDownload = (patient_name, medicines, date) => {
    ...
    doc.setFont("helvetica", "bold");
    doc.text("Patient Name: ", 40, 50);
    doc.setFont("helvetica", "normal");
    doc.text(patient_name, 100, 50);
    doc.setFont("helvetica", "bold");
