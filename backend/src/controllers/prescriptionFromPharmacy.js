...
const sendPrescriptionMedicinesToPharmacy = async (req, res) => {
  ...
  try {
    ...
    if (patient) {
      ...
      for (let i = 0; i < prescriptionIDs.length; i++) {
        let prescription = await Prescription.findOne({
          _id: prescriptionIDs[i].prescription_id,
        });

        console.log(prescription);
        //  if(prescription.isFilled === false){
        if (
          prescription.isFilled == false &&
      ...
      for (let i = 0; i < prescriptions.length; i++) {
        ...
        for (let j = 0; j < prescriptions[i].medicines.length; j++) {
          if (
            prescriptions[i].medicines[j].taken == false ||
            prescriptions[i].medicines[j].taken == undefined
          ) {
    ...
    return res.status(500).json({
      success: false,
      data: null,
      message: error.message || "Some error occurred while retrieving patient.",
    });
  }
};
    ...
    if (patient) {
      ...
      for (var medicine of medicines) {
        ...
        for (let j = 0; j < prescriptions.length; j++) {
          for (let k = 0; k < prescriptions[j].medicines.length; k++) {
            if (
              medicine === prescriptions[j].medicines[k].medicineName &&
              ...
              let prescription = await Prescription.findByIdAndUpdate(
                ...
                {
                  arrayFilters: [{ "elem.medicineName": medicine }],
                  new: true,
                }
              );
      ...
      for (let perO of prescriptions) {
        for (let j = 0; j < perO.medicines.length; j++) {
          if (perO.medicines[j].taken === false || perO.medicines[j].taken === undefined) {
            break;
          }
